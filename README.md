...
- поток Luna SSE → законченные фразы → быстрый Fish Audio HTTP PCM;
- раздельные метрики VAD, STT, снимка, первого/полного ответа модели, Fish и
  времени до первого звука;
...
- Каждый OpenRouter-запрос запрещает маршруты, допускающие сбор prompt-данных;
  `provider_zdr` дополнительно ограничивает маршрут только ZDR-endpoint'ами, если
  выбранная модель их поддерживает.
- Fish Audio получает только готовые к произнесению фразы ответа, без запроса, микрофонного
  аудио и снимка экрана.
...
- `model`, `provider`, `service_tier` — маршрут основной модели;
- `provider_data_collection` и `provider_zdr` — политика хранения у endpoint;
- `fish_*` — голос и потоковая озвучка.

Рабочий лог записывается в `berangaria-agent.log` с ротацией. По умолчанию он
содержит только технические метаданные и задержки; транскрипты и ответы остаются
в памяти GUI. Опция `log_content: true` включает полный текст только для
осознанной локальной диагностики. Лог никогда не публикуется в Git.
...
Windows CI выполняет тесты, Ruff, `py_compile`, Bandit, аудит зависимостей,
проверку BAT-launcher и сборку пакета на Python 3.11.
